...
config_path = '../NER/BERT/models/chinese_L-12_H-768_A-12/bert_config.json'
checkpoint_path = '../NER/BERT/models/chinese_L-12_H-768_A-12/bert_model.ckpt'
dict_path = '../NER/BERT/models/chinese_L-12_H-768_A-12/vocab.txt'
...
train_data = json.load(open('../datasets/re_1/train_data_me.json'))
print("\n\ntrain date len: {}\n\n".format(len(train_data)))
dev_data = json.load(open('../datasets/re_1/dev_data_me.json'))
id2predicate, predicate2id = json.load(
    open('../datasets/re_1/all_50_schemas_me.json'))
...
train_data = [total_data[j]
              for i, j in enumerate(random_order) if i % 8 != mode]
    ...
    def __iter__(self):
        while True:
            ...
            for i in idxs:
                ...
                items = {}
                for sp in d['spo_list']:
                    sp = (tokenizer.tokenize(sp[0])[
                          1:-1], sp[1], tokenizer.tokenize(sp[2])[1:-1])
                ...
                if items:
                    ...
                    k1 = choice(k1)
                    k2 = choice(k2[k2 >= k1])
                    o1, o2 = np.zeros((len(tokens), num_classes)), np.zeros(
                        (len(tokens), num_classes))
...
if __name__ == '__main__':
    ...
    train_model.fit(train_D.__iter__(),
